...
const checkCommand = 'adb shell "[ -f /data/user/0/host.exp.exponent/files/SQLite/habitosdeleitura.db ] && echo exists"';

console.log('Checking if database exists on device...');
exec(checkCommand, (checkError, checkStdout) => {
  if (!checkStdout.includes('exists')) {
    console.error('Database file not found on device');
    return;
  }
